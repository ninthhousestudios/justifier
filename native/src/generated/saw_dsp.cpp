...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn SawDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class SawDSP : public dsp {
 ...
 public:
	...
	SawDSP(const SawDSP&) = default;
	
	virtual ~SawDSP() = default;
	
	SawDSP& operator=(const SawDSP&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn SawDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp4 = std::max<float>(1.1920929e-07f, std::fabs(fSlow13 * fRec7[0]));
			float fTemp5 = fRec5[1] + fConst4 * fTemp4;
			float fTemp6 = fTemp5 + -1.0f;
			int iTemp7 = fTemp6 < 0.0f;
			fRec5[0] = ((iTemp7) ? fTemp5 : fTemp6);
