...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn Brown_noiseDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __Brown_noiseDSP_H__
#define  __Brown_noiseDSP_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS Brown_noiseDSP
#endif
...
static float Brown_noiseDSP_faustpower2_f(float value) {
	return value * value;
}

class Brown_noiseDSP : public dsp {
 ...
 public:
	...
	Brown_noiseDSP(const Brown_noiseDSP&) = default;
	
	virtual ~Brown_noiseDSP() = default;
	
	Brown_noiseDSP& operator=(const Brown_noiseDSP&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn Brown_noiseDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
		...
		m->declare("filename", "brown_noise.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		...
		fConst4 = std::tan(1570.7964f / fConst0);
		fConst5 = 2.0f * (1.0f - 1.0f / Brown_noiseDSP_faustpower2_f(fConst4));
	...
	virtual Brown_noiseDSP* clone() {
		return new Brown_noiseDSP(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("brown_noise");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec6[0] = 0.5221894f * fRec6[3] + 4.656613e-10f * static_cast<float>(iRec7[0]) + 2.494956f * fRec6[1] - 2.0172658f * fRec6[2];
			fRec5[0] = 0.049922034f * fRec6[0] + 0.0506127f * fRec6[2] - (0.095993534f * fRec6[1] + 0.004408786f * fRec6[3] + fConst8 * (fConst7 * fRec5[2] + fConst5 * fRec5[1]));
			float fTemp4 = fConst8 * (fRec5[2] + fRec5[0] + 2.0f * fRec5[1]);
			fRec4[0] = fTemp4 - (fRec4[2] * ((fTemp2 - fTemp1) / fTemp0 + 1.0f) + 2.0f * fRec4[1] * (1.0f - 1.0f / Brown_noiseDSP_faustpower2_f(fTemp0))) / fTemp3;
			float fTemp5 = (fRec4[2] + fRec4[0] + 2.0f * fRec4[1]) / fTemp3;
			float fTemp6 = (fRec4[0] - fRec4[2]) / (fTemp0 * fTemp3);
			fRec8[0] = fSlow13 + fConst2 * fRec8[1];
			output0[i0] = static_cast<FAUSTFLOAT>(fRec8[0] * ((iSlow8) ? ((iSlow12) ? fTemp4 - fTemp6 : fTemp6) : ((iSlow9) ? fTemp4 - fTemp5 : fTemp5)) * std::max<float>(0.0f, std::min<float>(fSlow4 * fRec1[0], std::max<float>(fSlow6 * (fSlow3 - fRec1[0]) + 1.0f, fSlow5)) * (1.0f - fSlow2 * static_cast<float>(iRec0[0]))));
			fVec0[1] = fVec0[0];
			iRec0[1] = iRec0[0];
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
