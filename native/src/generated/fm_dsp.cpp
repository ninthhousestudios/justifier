...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn FmDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class FmDSPSIG0 {
  ...
  public:
	...
	int getNumInputsFmDSPSIG0() {
		return 0;
	}
	int getNumOutputsFmDSPSIG0() {
		return 1;
	}
	
	void instanceInitFmDSPSIG0(int sample_rate) {
	...
	void fillFmDSPSIG0(int count, float* table) {
...
static FmDSPSIG0* newFmDSPSIG0() { return (FmDSPSIG0*)new FmDSPSIG0(); }
static void deleteFmDSPSIG0(FmDSPSIG0* dsp) { delete dsp; }

static float FmDSP_faustpower2_f(float value) {
	return value * value;
}
static float ftbl0FmDSPSIG0[65536];

class FmDSP : public dsp {
 ...
 public:
	...
	FmDSP(const FmDSP&) = default;
	
	virtual ~FmDSP() = default;
	
	FmDSP& operator=(const FmDSP&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn FmDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	static void classInit(int sample_rate) {
		FmDSPSIG0* sig0 = newFmDSPSIG0();
		sig0->instanceInitFmDSPSIG0(sample_rate);
		sig0->fillFmDSPSIG0(65536, ftbl0FmDSPSIG0);
		deleteFmDSPSIG0(sig0);
	}
	...
	virtual void instanceResetUserInterface() {
		...
		fEntry9 = static_cast<FAUSTFLOAT>(4.4e+02f);
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addNumEntry("amp", &fEntry12, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.001f));
		...
		ui_interface->addNumEntry("detune", &fEntry10, FAUSTFLOAT(0.0f), FAUSTFLOAT(-1e+02f), FAUSTFLOAT(1e+02f), FAUSTFLOAT(0.01f));
		...
		ui_interface->addNumEntry("freq", &fEntry9, FAUSTFLOAT(4.4e+02f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(2e+04f), FAUSTFLOAT(0.01f));
		ui_interface->addNumEntry("gate", &fEntry0, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1.0f));
		ui_interface->addNumEntry("mod_index", &fEntry11, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(25.0f), FAUSTFLOAT(0.01f));
		ui_interface->addNumEntry("mod_ratio", &fEntry8, FAUSTFLOAT(1.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(0.01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow13 = fConst1 * static_cast<float>(fEntry9);
		float fSlow14 = fConst4 * std::pow(2.0f, 0.00083333335f * static_cast<float>(fEntry10));
		float fSlow15 = fConst1 * static_cast<float>(fEntry11);
		int iSlow16 = iSlow7 >= 3;
		float fSlow17 = fConst1 * static_cast<float>(fEntry12);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec8[0] = fSlow12 + fConst2 * fRec8[1];
			fRec9[0] = fSlow13 + fConst2 * fRec9[1];
			float fTemp5 = ((iTemp4) ? 0.0f : fRec7[1] + fSlow14 * fRec9[0] * fRec8[0]);
			fRec7[0] = fTemp5 - std::floor(fTemp5);
			fRec10[0] = fSlow15 + fConst2 * fRec10[1];
			float fTemp6 = ((iTemp4) ? 0.0f : fRec6[1] + fSlow14 * fRec9[0] * (fRec10[0] * ftbl0FmDSPSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec7[0]), 65535))] + 1.0f));
			fRec6[0] = fTemp6 - std::floor(fTemp6);
			float fTemp7 = ftbl0FmDSPSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec6[0]), 65535))];
			fRec4[0] = fTemp7 - (fRec4[2] * ((fTemp2 - fTemp1) / fTemp0 + 1.0f) + 2.0f * fRec4[1] * (1.0f - 1.0f / FmDSP_faustpower2_f(fTemp0))) / fTemp3;
			float fTemp8 = fRec4[2] + fRec4[0] + 2.0f * fRec4[1];
			float fTemp9 = fTemp0 * fTemp3;
			float fTemp10 = fRec4[0] - fRec4[2];
			fRec11[0] = fSlow17 + fConst2 * fRec11[1];
			output0[i0] = static_cast<FAUSTFLOAT>(fRec11[0] * ((iSlow8) ? ((iSlow16) ? fTemp7 * (1.0f - fTemp10 / fTemp9) : fTemp7 * fTemp10 / fTemp9) : ((iSlow9) ? -(fTemp7 * (fTemp8 / fTemp3 - fTemp7)) : fTemp7 * fTemp8 / fTemp3)) * std::max<float>(0.0f, std::min<float>(fSlow4 * fRec1[0], std::max<float>(fSlow6 * (fSlow3 - fRec1[0]) + 1.0f, fSlow5)) * (1.0f - fSlow2 * static_cast<float>(iRec0[0]))));
			iVec0[1] = iVec0[0];
			fVec1[1] = fVec1[0];
			iRec0[1] = iRec0[0];
			fRec1[1] = fRec1[0];
