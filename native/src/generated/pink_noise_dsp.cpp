...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn Pink_noiseDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __Pink_noiseDSP_H__
#define  __Pink_noiseDSP_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS Pink_noiseDSP
#endif
...
static float Pink_noiseDSP_faustpower2_f(float value) {
	return value * value;
}

class Pink_noiseDSP : public dsp {
 ...
 public:
	...
	Pink_noiseDSP(const Pink_noiseDSP&) = default;
	
	virtual ~Pink_noiseDSP() = default;
	
	Pink_noiseDSP& operator=(const Pink_noiseDSP&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn Pink_noiseDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
		...
		m->declare("filename", "pink_noise.dsp");
	...
	virtual Pink_noiseDSP* clone() {
		return new Pink_noiseDSP(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("pink_noise");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec5[0] = 0.5221894f * fRec5[3] + 4.656613e-10f * static_cast<float>(iRec6[0]) + 2.494956f * fRec5[1] - 2.0172658f * fRec5[2];
