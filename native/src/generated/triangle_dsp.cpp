...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn TriangleDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
static float TriangleDSP_faustpower2_f(float value) {
	return value * value;
}
...
class TriangleDSP : public dsp {
 ...
 public:
	...
	TriangleDSP(const TriangleDSP&) = default;
	
	virtual ~TriangleDSP() = default;
	
	TriangleDSP& operator=(const TriangleDSP&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn TriangleDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp4 = std::max<float>(fSlow13 * fRec6[0], 23.44895f);
			float fTemp5 = std::max<float>(2e+01f, std::fabs(fTemp4));
			float fTemp6 = ((1 - iVec0[1]) ? 0.0f : fRec7[1] + fConst4 * fTemp5);
			fRec7[0] = fTemp6 - std::floor(fTemp6);
			float fTemp7 = TriangleDSP_faustpower2_f(2.0f * fRec7[0] + -1.0f);
			fVec2[0] = fTemp7;
