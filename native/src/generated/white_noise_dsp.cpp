 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp4 = 4.656613e-10f * static_cast<float>(iRec5[0]);
			fRec4[0] = fTemp4 - (fRec4[2] * ((fTemp2 - fTemp1) / fTemp0 + 1.0f) + 2.0f * fRec4[1] * (1.0f - 1.0f / White_noiseDSP_faustpower2_f(fTemp0))) / fTemp3;
			...
			output0[i0] = static_cast<FAUSTFLOAT>(fRec6[0] * ((iSlow8) ? ((iSlow12) ? fTemp4 - fTemp6 : fTemp6) : ((iSlow9) ? fTemp4 - fTemp5 : fTemp5)) * std::max<float>(0.0f, std::min<float>(fSlow4 * fRec1[0], std::max<float>(fSlow6 * (fSlow3 - fRec1[0]) + 1.0f, fSlow5)) * (1.0f - fSlow2 * static_cast<float>(iRec0[0]))));
			fVec0[1] = fVec0[0];
			iRec0[1] = iRec0[0];
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
