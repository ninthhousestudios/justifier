...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn SineDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __SineDSP_H__
#define  __SineDSP_H__
...
class SineDSPSIG0 {
  ...
  public:
	...
	int getNumInputsSineDSPSIG0() {
		return 0;
	}
	int getNumOutputsSineDSPSIG0() {
		return 1;
	}
	
	void instanceInitSineDSPSIG0(int sample_rate) {
	...
	void fillSineDSPSIG0(int count, float* table) {
...
static SineDSPSIG0* newSineDSPSIG0() { return (SineDSPSIG0*)new SineDSPSIG0(); }
static void deleteSineDSPSIG0(SineDSPSIG0* dsp) { delete dsp; }

static float SineDSP_faustpower2_f(float value) {
	return value * value;
}
static float ftbl0SineDSPSIG0[65536];

class SineDSP : public dsp {
 ...
 public:
	...
	SineDSP(const SineDSP&) = default;
	
	virtual ~SineDSP() = default;
	
	SineDSP& operator=(const SineDSP&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn SineDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	static void classInit(int sample_rate) {
		SineDSPSIG0* sig0 = newSineDSPSIG0();
		sig0->instanceInitSineDSPSIG0(sample_rate);
		sig0->fillSineDSPSIG0(65536, ftbl0SineDSPSIG0);
		deleteSineDSPSIG0(sig0);
	}
	...
	virtual SineDSP* clone() {
		return new SineDSP(*this);
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow13 = fConst4 * std::pow(2.0f, 0.00083333335f * static_cast<float>(fEntry9));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec7[0] = fSlow12 + fConst2 * fRec7[1];
			float fTemp4 = ((1 - iVec0[1]) ? 0.0f : fRec6[1] + fSlow13 * fRec7[0]);
