...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn PulseDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __PulseDSP_H__
#define  __PulseDSP_H__
...
static float PulseDSP_faustpower2_f(float value) {
	return value * value;
}

class PulseDSP : public dsp {
 ...
 public:
	...
	PulseDSP(const PulseDSP&) = default;
	
	virtual ~PulseDSP() = default;
	
	PulseDSP& operator=(const PulseDSP&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn PulseDSP -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void instanceResetUserInterface() {
		...
		fEntry8 = static_cast<FAUSTFLOAT>(0.5f);
	...
	virtual PulseDSP* clone() {
		return new PulseDSP(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("pulse");
		...
		ui_interface->addNumEntry("duty", &fEntry8, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.01f), FAUSTFLOAT(0.99f), FAUSTFLOAT(0.01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow16 = fConst1 * static_cast<float>(fEntry11);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp4 = ((1 - iVec0[1]) ? 0.0f : fRec6[1] + fSlow14 * fRec7[0]);
			fRec6[0] = fTemp4 - std::floor(fTemp4);
			float fTemp5 = 2.0f * static_cast<float>(fRec6[0] <= fRec5[0]);
			fRec4[0] = fTemp5 + (-1.0f - (fRec4[2] * ((fTemp2 - fTemp1) / fTemp0 + 1.0f) + 2.0f * fRec4[1] * (1.0f - 1.0f / PulseDSP_faustpower2_f(fTemp0))) / fTemp3);
			...
			output0[i0] = static_cast<FAUSTFLOAT>(fRec8[0] * ((iSlow8) ? ((iSlow15) ? fTemp5 + (-1.0f - fTemp8) : fTemp8) : ((iSlow9) ? -(fTemp7 * (fTemp6 / fTemp3 - fTemp7)) : fTemp7 * fTemp6 / fTemp3)) * std::max<float>(0.0f, std::min<float>(fSlow4 * fRec1[0], std::max<float>(fSlow6 * (fSlow3 - fRec1[0]) + 1.0f, fSlow5)) * (1.0f - fSlow2 * static_cast<float>(iRec0[0]))));
			iVec0[1] = iVec0[0];
			fVec1[1] = fVec1[0];
			iRec0[1] = iRec0[0];
			fRec1[1] = fRec1[0];
